...
exports.machine_list = async function (req, res, next) {
  try {
    ...
    const machine_list = await Machine.findAll({
      // on récupere la liste des machines en la stockant dans variable
      order: [['nom', 'ASC']], // machine_list
    });
    res.render('machine_list', {
      title: 'Voici la liste des machines disponibles :',
      machine_list,
    });
  } catch (error) {
    next(error);
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        const machine = await Machine.build({
          // on créer l'objet
          nom: req.body.nom,
...
exports.machine_update_get = async function (req, res, next) {
  try {
    ...
    if (machine === null) {
      // si on ne trouve pas la machine
...
exports.machine_update_post = [
  ...
  body('nom', 'Veuillez indiquer le nom de la machine.')
    .trim()
    .notEmpty()
    .escape(),
...
exports.machine_delete_get = async function (req, res, next) {
  try {
    ...
    const machine = await Machine.findByPk(req.params.id, {
      // on récupère machine à partir de l'id et..
      include: Utilisation, //  les utilisations qui y sont liées
    });
    if (machine === null) {
      // si machine non trouvée en bdd, redirection
...
exports.machine_delete_post = async function (req, res, next) {
  try {
    ...
    if (machine === null) {
      // si on trouve pas la machine
      next(createError(404, 'Machine non trouvée')); // on renvoie erreur 404
    } else if (machine.utilisations.length > 0) {
      // si il reste une utilisation
      res.render(
        'machine_delete', // on revient au formulaire
