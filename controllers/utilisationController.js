...
exports.utilisation_list = async function (req, res, next) {
  try {
    ...
    let utilisation_lis;
    if (userPermissions.includes('lireToutesLesUtilisations')) {
      utilisation_list = await Utilisation.findAll({
        // on récupere la liste des utilisations et on la stocke
...
exports.utilisation_detail = async function (req, res, next) {
  try {
    ...
    const utilisation = await Utilisation.findByPk(utilisationId, {
      // et on lance une recherche en BDD
      include: [Utilisateur, Machine],
    });
    if (utilisation !== null) {
      res.render('utilisation_detail', {
        title: 'Details des utilisations',
        utilisation,
        user: req.user,
      });
    } else {
...
exports.utilisation_create_get = async function (req, res, next) {
  try {
    ...
    if (req.query.machineid) {
      // 49 - 54 : si on a reçu dans l'url un id de machine ou utilisateur, on va le stocker
      ...
      // correspondante sur base de l'id
  ...
  async function (req, res, next) {
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        const recupMachine = await Machine.findByPk(
          req.body.machineidhidden || req.body.machineid
        );
        const utilisation = await Utilisation.build({
          // sinon on va créer la nouvelle utilisation
        ...
        const recupUtilisateur = await Utilisateur.findByPk(
          req.body.utilisateuridhidden || req.body.utilisateurid
        );
